...
- Неоднозначные поля вроде `Кто принимает финальное решение`, `Статус запуска`, `Для кого продукт`, `Что сейчас оптимизируем` и `Ключевые метрики` заменены на явные, но компактные контексты: минимальная карта ответственности за цель, основные роли клиента, статус запуска по слоям, глобальная и ближайшая цель, классы метрик и источники правды по типам.
...
- Последняя wording-правка по ambiguity пока проверена только markdown/git-level; перед merge желательно повторить API behavior baseline.
