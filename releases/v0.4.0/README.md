...
- Неоднозначные поля вроде `Кто принимает финальное решение`, `Статус запуска`, `Для кого продукт`, `Что сейчас оптимизируем` и `Ключевые метрики` заменены на явные контексты: карта ответственности за цель, роли клиента, статус запуска по слоям, контекст оптимизации, классы метрик и источники правды по типам.
