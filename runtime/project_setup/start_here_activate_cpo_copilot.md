...
```markdown
## Один следующий вопрос
Уточни недостающие decision rights по ближайшей цели?

Owner внутри нашей команды:
Какие решения owner может принимать сам:
Какие решения требуют отдельного подтверждения:
Недостающая роль клиента, если её ещё не назвали:
```
