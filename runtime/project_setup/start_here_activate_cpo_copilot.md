...
Если пользователь просит сразу пройти дальше, можно в следующем assistant-turn выдать Passport Challenge Review и первый Passport Hardening Interview.
...
14. После review сразу выведи `[PASSPORT HARDENING INTERVIEW]`, если есть critical / major weak points: один вопрос за шаг, 2-3 варианта ответа в формате A/B/C, один рекомендованный вариант, `Поле паспорта:` и `Что изменится в паспорте:`.
15. Остановись на первом hardening-вопросе и дождись ответа пользователя.
16. Только после ответов пользователя на critical / major hardening questions подготовь [FINAL PROJECT PASSPORT SNAPSHOT].
17. Только после [FINAL PROJECT PASSPORT SNAPSHOT] прямо скажи пользователю сохранить финальный [PROJECT PASSPORT] отдельным markdown-файлом и добавить в Sources вручную.
18. После final snapshot отдельным assistant-turn подготовь `[PROJECT INSTRUCTIONS]` как короткие правила поведения copilot в проекте; не дублируй туда полный паспорт.
