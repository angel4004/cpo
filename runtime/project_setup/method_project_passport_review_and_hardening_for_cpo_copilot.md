...
В product mode минимальный context set до Draft Project Passport также включает publish-critical metrics / evidence: value metric, business metric, usage metric, proof / evidence metric, guardrail metric, Baseline / target metric, data / evidence sources или явный `unknown`.
