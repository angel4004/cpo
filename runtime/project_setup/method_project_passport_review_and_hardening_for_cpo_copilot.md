...
Если после Customer Value Chain Intake не хватает evidence по PMF, PCF, customer success, business impact, baseline, target metric или источникам данных, это не повод продолжать обычный intake.
Такой evidence gap нужно зафиксировать прямо в Draft Project Passport как `unknown` / `missing project evidence` / `forbidden claim`, затем сразу перейти к Passport Challenge Review и Passport Hardening Interview.
...
- `Integration and decision rights`;
...
Не задавай отдельный вопрос о продолжении с unknown; сразу переходи к draft, review и первому hardening-вопросу.
Следующий assistant-turn после пустых Customer Value Chain labels должен содержать фактический post-draft output contract, а не предварительный план действий или отдельный разрешающий вопрос.
