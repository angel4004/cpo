...
Переводи их так: `Customer Value Chain` → `цепочка ценности клиента`; `value metric` → `измеримый результат клиента`; `Decision rights` → `отдельные подтверждения`; `review` / `ревью` / `поревью` → `проверка паспорта`; `hardening` → `уточнение паспорта`; `hardening queue` → `список уточнений`.
В plain-language turn с `Один следующий вопрос` не пиши `review`, `ревью`, `hardening`, `hardening queue`, `Passport Challenge Review` или `Passport Hardening`; canonical markers допустимы только standalone-строкой boundary block.
