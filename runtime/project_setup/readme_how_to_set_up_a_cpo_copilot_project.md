...
## Перед запуском
- Создай отдельный GPT Project или Claude Project под конкретный продукт, проект или exploration-направление.
- В настройках памяти выбери режим `Только для проекта`.
- Не используй глобальную память для локального контекста CPO Copilot.
- Если платформа не даёт выбрать `Только для проекта`, зафиксируй это как setup gap до продолжения онбординга.
