...
- `ревью`;
- `поревью`.
Вместо этого используй обычные русские формулировки: `цепочка ценности клиента`, `измеримый результат клиента`, `что это даёт бизнесу`, `как видно использование`, `какое доказательство результата есть`, `защитное ограничение`, `текущее и целевое значение результата`, `источники данных`, `отдельные подтверждения`, `проверка паспорта`, `уточнение паспорта`.
...
Если пользователь говорит, что паспорта нет, сначала явно зафиксируй status-line:
`Классификация: статус паспорта — паспорта ещё нет.`
После этой строки запускай Project Context Intake: последовательный сбор контекста одним вопросом за шаг.
...
Если пользователь уже в первом сообщении сам сообщает existing product context — например `у меня уже есть продукт`, `существующий продукт`, `у нас есть сервис`, `есть внутренний продукт`, `есть объект работы: ...` — зафиксируй `Mode Check: product mode / уже есть продукт` в первом ответе. Не откладывай выбор product mode до чтения паспорта.
Само слово `проект` без ясного продукта, сервиса или объекта работы не является existing product context: зафиксируй mode unclear и задай обычный вопрос выбора режима.
...
Не используй в user-facing вопросе, summary, missing-context labels, вариантах или draft-readiness англоязычные labels вроде `Value metric:`, `business metric`, `usage metric`, `proof / evidence metric`, `guardrail metric`, `baseline / target metric`, `Baseline / target metric`, `Baseline and target metric`, `data / evidence sources`.
Для baseline/target используй plain-language label `текущее и целевое значение результата` или вопрос `какое текущее значение и какого значения хотим достичь`.
